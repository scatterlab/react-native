...
- **글자수 제한에서 조합이 막히는 것은 iOS 플랫폼 동작이고 결함이 아니다**(다른 앱도 동일). 조합 중인 글자가 `text`의 문자 수를 차지하므로 상한에서 `allowedLength`가 0이 되어 키스트로크가 폐기된다 — 한국어 `ㅇ`+`ㅏ`, 일본어 `n`+`i`, 병음 모두 같은 지점에서 멈춘다.

  **fork에서 고치려 시도했고 폐기했다**(`74462d03f8e` → `1b9cfe7c19c`). 접근은 상한에서 caret 위치 단일 유닛 삽입만 통과시키고 `-textInputDidChange`에서 길이를 정산하는 것이었다 — 한국어에 `markedTextRange`가 없다는 제약을 "지금 조합 중인가" 대신 "정착된 길이가 상한을 넘는가"로 우회하는 유일한 길이었고, `-_updateState` 앞에서 정산해 `onChangeText` 계약도 지켰다. 그런데 실기기에서 다른 버그가 생기고 동작이 애매했다: 상한 부근의 편집 경로(붙여넣기, 중간 삽입, 삭제 후 재입력, 확정 취소)가 많아 부작용을 좁히기 어렵다.

  앱 레벨이 필요하면 네이티브에 `maxLength + 여유`를 주고 실제 제한을 `onChangeText`에서 거는 쪽이 같은 효과를 계약 위반 없이 낸다. fork diff도 늘지 않는다.
